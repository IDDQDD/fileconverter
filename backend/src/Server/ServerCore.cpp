...
ServerCore::ServerCore(net::io_context &ioc, const ServerSettings &settings,
                       const ConnectionSettings &conn_settings,
                       std::shared_ptr<PluginManager> pm)
    : server_settings_(settings), connection_settings_(conn_settings), plugin_manager_(pm),
...
void ServerCore::handle_accept(beast::error_code ec, tcp::socket socket) {
    if(ec) {
        ...
        std::make_shared<RequestHandler>(std::move(socket), connection_settings_, plugin_manager_)->handle_request();
    }
